...
def visualize_pdf_parsing(pdf_path: str, parser_url: str, output_path: str, page_num: int = 1):
    print(f"==============================================")
    print(f" DOCLING PARSER VISUALIZATION")
    ...
    # 2. PDFの指定ページを画像に変換
    print(f"[2] Converting PDF page {page_num} to image...")
    try:
        pages = convert_from_path(pdf_path, first_page=page_num, last_page=page_num)
        if not pages:
            print(f"    -> [ERROR] Failed to extract page {page_num} from PDF.")
            return
        page_img = pages[0].convert("RGB")
        draw = ImageDraw.Draw(page_img, "RGBA")
        print(f"    -> [PASS] Image converted (Size: {page_img.width}x{page_img.height})")
    except Exception as e:
    ...
    if len(page_data_list) < page_num:
        print(f"    -> [ERROR] Parser did not return data for page {page_num}.")
        return
    ...
    try:
        ...
        print(f"==============================================")
        print(f" [SUCCESS] Visualized image saved to: {output_path}")
        print(f"==============================================")
    except Exception as e:
        print(f"[ERROR] Failed to save image: {e}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize Docling Parser Output")
    ...
    parser.add_argument("--out", type=str, default="visualized_page.png", help="Output image file path")
    parser.add_argument("--page", type=int, default=1, help="Page number to visualize (1-indexed)")
